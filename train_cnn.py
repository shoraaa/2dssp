...
def tiles_to_embs_train(tiles_np, alphabet: int, tile_cnn: nn.Module, device) -> torch.Tensor:
    """
    tiles_np: (M, n, n) int np.ndarray in [0, alphabet)
    returns: (M, d_tile) with grad
    """
    x = torch.as_tensor(tiles_np, device=device, dtype=torch.long)           # (M,n,n)
    x = F.one_hot(x, num_classes=alphabet).float().permute(0, 3, 1, 2)      # (M,C,n,n)
    x = x.unsqueeze(0)                                                       # (1,M,C,n,n)
    embs = tile_cnn(x).squeeze(0)                                            # (M,d_tile)
    return embs

@torch.no_grad()
def tiles_to_embs_eval(tiles_np, alphabet: int, tile_cnn: nn.Module, device) -> torch.Tensor:
    x = torch.as_tensor(tiles_np, device=device, dtype=torch.long)           # (M,n,n)
    x = F.one_hot(x, num_classes=alphabet).float().permute(0, 3, 1, 2)      # (M,C,n,n)
    x = x.unsqueeze(0)                                                       # (1,M,C,n,n)
    return tile_cnn(x).squeeze(0)                                            # (M,d_tile)
...
def train_reinforce(
    ...
    opt = AdamW(list(model.parameters()) + list(tile_cnn.parameters()), lr=lr)
    ...
    for ep in tbar:
        ...
        # Timing aggregators
        total_tile_emb_time = 0.0
        total_build_batch_time = 0.0
        ...
        for idx in idxs:
            ...
            # Time tile embedding computation
            start_time = time.time()
            tile_cnn.train()  # ensure train mode (even though we don't use BN/Dropout, makes intent clear)
            tile_embs = tiles_to_embs_train(env.tiles, env.alphabet, tile_cnn, device)
            total_tile_emb_time += time.time() - start_time

            logps, rewards, final_m, steps, ent_sum, build_batch_time, model_inference_time, env_step_time = rollout_episode(
                model=model,
                env=env,
                tile_embs=tile_embs,
                device=device,
            ...
            # Accumulate timing
            total_build_batch_time += build_batch_time
        ...
        # Time gradient computation and update
        ...
        opt.step()
        total_gradient_time = time.time() - grad_start_time
        ...
        if use_wandb:
            ...
            wandb.log({
                ...
                # Timing metrics
                "timing/epoch_total_time": epoch_total_time,
                "timing/tile_embedding_time": total_tile_emb_time,
                "timing/build_batch_time": total_build_batch_time,
                "timing/model_inference_time": total_model_inference_time,
                "timing/env_step_time": total_env_step_time,
                "timing/gradient_time": total_gradient_time,
                # Timing per episode
                "timing/tile_embedding_time_per_episode": total_tile_emb_time / batch_size,
                "timing/build_batch_time_per_episode": total_build_batch_time / batch_size,
                "timing/model_inference_time_per_episode": total_model_inference_time / batch_size,
                "timing/env_step_time_per_episode": total_env_step_time / batch_size,
                "epoch": ep,
            })

        # ---- Validation ----
        if ep % eval_every == 0:
            model.eval(); tile_cnn.eval()
            val_start_time = time.time()
            ms, stepss = [], []
            val_tile_emb_time = 0.0
            val_inference_time = 0.0
            
            for venv in val_envs:
                # Time validation tile embedding
                start_time = time.time()
                vemb = tiles_to_embs_eval(venv.tiles, venv.alphabet, tile_cnn, device)
                val_tile_emb_time += time.time() - start_time
                
                # Time validation inference
                start_time = time.time()
            ...
            if use_wandb:
                wandb.log({
                    "val/m": val_m_mean,
                    "val/steps": val_steps_mean,
                    # Validation timing metrics
                    "timing/val_total_time": val_total_time,
                    "timing/val_tile_embedding_time": val_tile_emb_time,
                    "timing/val_inference_time": val_inference_time,
                    "timing/val_tile_embedding_time_per_instance": val_tile_emb_time / len(val_envs),
                    "timing/val_inference_time_per_instance": val_inference_time / len(val_envs),
                    "epoch": ep
                })

        if ep % log_every == 0:
            ckpt_path = os.path.join(save_dir, f"ep{ep}.pt")
            torch.save({"model": model.state_dict(), "tile_cnn": tile_cnn.state_dict(), "opt": opt.state_dict()}, ckpt_path)

    final_path = os.path.join(save_dir, "final_reinforce.pt")
    torch.save({"model": model.state_dict(), "tile_cnn": tile_cnn.state_dict(), "opt": opt.state_dict()}, final_path)
...
def main():
    ...
    p.add_argument("--batch_size", type=int, default=64, help="episodes sampled per epoch")
    ...
    p.add_argument("--train_data_size", type=int, default=1024)
    p.add_argument("--val_data_size", type=int, default=16)
