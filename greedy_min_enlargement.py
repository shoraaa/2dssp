        ...
        for v in range(T):
            ...
            for dx in range(-(n-1), n):
                for dy in range(-(n-1), n):
                    ...
                    if ai0>=ai1 or aj0>=aj1: continue
                    bi0 = ai0 - dx; bj0 = aj0 - dy
                    bi1 = ai1 - dx; bj1 = aj1 - dy
                    Aov = A[ai0:ai1, aj0:aj1]; Bov = B[bi0:bi1, bj0:bj1]
...
def feasible_on_occupancy(tile: np.ndarray, x:int, y:int, occ: Dict[Coordinate,int])->bool:
    ...
    for i in range(n):
        for j in range(n):
            val=int(tile[i,j]); coord=(x+i, y+j)
            if coord in occ and occ[coord] != val: return False
    return True
...
def bbox_after_place(x:int,y:int,n:int,bbox:List[int]):
    ...
    m_old=max(xmax-xmin+1, ymax-ymin+1)
    m_new=max(nxmax-nxmin+1, nymax-ymin+1)
    return (m_new - m_old), m_new, [nxmin,nxmax,ymin,ymax]
...
def greedy_min_enlargement_once(tiles: List[np.ndarray], params: GreedyMinEnlargeParams)->Dict[str,Any]:
    ...
    for idx in range(1,T):
        ...
        best=None  # (inc, -overlap, rand, x, y, m_new)
        for (x,y) in posset:
            if not feasible_on_occupancy(tiles[v], x, y, occ): continue
            inc, m_new, _ = bbox_after_place(x,y,n,bbox)
            total_ov=0
            for u in placed_ids:
                ux,uy=placements[u]; dx=x-ux; dy=y-uy
                total_ov += ov_lookup.get((u,v,dx,dy), 0)
            key=(inc, -total_ov, random.random())
        ...
        if best is not None:
            ...
            fb=perimeter_candidates(v)
            if not fb:
                x,y = bbox[0]-n, bbox[2]-n
            else:
                choose=None
                for (px,py) in fb:
                    inc, m_new, _ = bbox_after_place(px,py,n,bbox)
                    key=(inc, random.random())
                    if choose is None or key<choose[0]:
                        choose=(key, px, py, m_new)
                _, x, y, _ = choose

        placements[v]=(x,y)
        write_to_occupancy(tiles[v], x, y, occ)
        _, _, bbox = bbox_after_place(x,y,n,bbox)
